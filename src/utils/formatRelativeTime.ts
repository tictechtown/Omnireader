function timeDifference(timestampInMs: number, locale: string = "en-US") {
...
export default function formatRelativeTime(dateStr: string | null) {
  if (!dateStr) {
    return null;
  }
  const displayedDate = new Date(dateStr);

  return (
    timeDifference(displayedDate.getTime()) ??
